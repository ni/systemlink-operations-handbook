...
The majority of SystemLink services use [MongoDB](https://www.mongodb.com) as the primary database. Default installations of SystemLink include MongoDB community Edition that launches when the SystemLink server boots. This is referred to as a [*single node*](#single-node-deployments) deployment.
...
1. Log into the desktop of the SystemLink app server with a user with administrator privileges.
...
NI recommends connecting SystemLink to replica set of three or more MongoDB members or MongoDB Atlas for all production deployments.
...
- When using MongoDB Atlas or [MongoDB Enterprise Advanced](https://www.mongodb.com/products/mongodb-enterprise-advanced), you have the option to encrypt the data stored within the database.
...
Replica sets provide a way to create redundancy in your database to mitigate against potential data loss. Refer to MongoDB for a [tutorial on deploying replica sets](https://docs.mongodb.com/manual/tutorial/deploy-replica-set/).
...
MongoDB Atlas provides simpler setup and adminstration compared to self hosted replica sets and stand alone deployments. MongoDB Atlas is a fully managed PaaS from MongoDB.
